...
package com.tongyuan.android.zhiquleyuan.zxing.encode;
...
final class EncodeThread extends Thread {
    ...
    public void run() {
        try {
            ...
            LogUtil.e(TAG, "Could not encode barcode : " + e.toString());
            Message message = Message.obtain(handler, R.id.encode_failed);
            message.sendToTarget();
        } catch (IllegalArgumentException e) {
            LogUtil.e(TAG, "Could not encode barcode :m " + e.toString());
            Message message = Message.obtain(handler, R.id.encode_failed);
            message.sendToTarget();
        }
    }
